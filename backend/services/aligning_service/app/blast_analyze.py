...
def parse_blast_results(result_file: Union[str, Path], return_mutations: bool = False) -> Optional[Tuple[List[Dict], List[Dict]]]:
    """
            ...
            for i, blast_record in enumerate(blast_records):
                ...
                for alignment in blast_record.alignments:
                    for hsp in alignment.hsps:
                        ...
                        logger.info(f"  Identities: {hsp.identities}/{hsp.align_length} ({identity_pct:.1f}%)")
                        logger.info(f"  Coverage: {hsp.align_length}/{query_length} ({coverage_pct:.1f}%)")
                        
                        # Extract mutations
                        mutations = extract_mutations_from_hsp(hsp, chromosome, query_id, subject_id)
    ...
    for i, (q, s) in enumerate(zip(hsp.query, hsp.sbjct)):
        ...
        if q != s and q != '-' and s != '-':
            mutation = {
                ...
                'reference': s,
                'query': q,
                'context': hsp.sbjct[max(0, i-5):i] + '[' + s + ']' + hsp.sbjct[i+1:min(i+6, len(hsp.sbjct))],
                'query_id': query_id,
                'subject_id': subject_id,
        ...
        # Handle insertions in query (gaps in reference)
            ...
            mutation = {
                ...
                'reference': '-',
                'query': q,
                'context': hsp.sbjct[max(0, i-5):i] + '[-]' + hsp.sbjct[i+1:min(i+6, len(hsp.sbjct))],
                'query_id': query_id,
                'subject_id': subject_id,
                'alignment_start': hsp.sbjct_start,
                'alignment_end': hsp.sbjct_end,
...
def log_mutation_summary(mutations: List[Dict]) -> None:
    ...
    for mut in mutations[:5]:
        logger.info(f"    {mut['type']} at Chr {mut['chromosome']} position {mut['position']}: "
...
def main():
    ...
    try:
        ...
        with open(summary_csv, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['file_id', 'total_mutations', 'SNP_count', 'INS_count', 'DEL_count', 'ERROR_count'])
