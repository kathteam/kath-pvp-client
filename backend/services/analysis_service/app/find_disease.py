...
def convert_coordinates(chromosome, position, from_build="GRCh38", to_build="GRCh37"):
    ...
    try:
        ...
        # No conversion found
        logger.warning(f"No manual conversion available for {key}. Install PyLiftover for better results.")
        conversion_cache[cache_key] = (None, None)
        save_cache("liftover", conversion_cache)
        return (None, None)

def find_disease_from_variant(chromosome, position, ref_allele, alt_allele, email="your.email@example.com", 
                               source_build="GRCh38"):
    """
    ...
    if not result and source_build == "GRCh38":
        ...
        if conv_chrom and conv_pos:
            logger.info(f"Converted coordinates: chr{chromosome}:{position} (GRCh38) -> chr{conv_chrom}:{conv_pos} (GRCh37)")
...
def query_clinvar(chromosome, position, ref_allele, alt_allele):
    ...
    try:
        ...
        if not summary_results or not summary_results.get("DocumentSummarySet", {}).get("DocumentSummary", []):
        ...
        if fetch_data and "result" in fetch_data:
            ...
            if "clinical_significance" in variant_info:
                clinical_significance = variant_info["clinical_significance"].get("description", "Unknown")
        ...
        result = {
            ...
            "diseases": disease_names if disease_names else ["No specific disease information available"],
...
def format_disease_output(result: Dict[str, Any]) -> None:
    ...
    print(f"Variant: chr{result['chromosome']}:{result['position']} {result['ref_allele']}>{result['alt_allele']}")
...
if __name__ == "__main__":
    ...
    examples = [
        ...
        {"chrom": "17", "pos": "43045629", "ref": "A", "alt": "G", "disease": "Hereditary breast and ovarian cancer syndrome"},
    ...
    print(f"Querying ClinVar for variant chr{chromosome}:{position} {ref_allele}>{alt_allele} (GRCh38)")
    ...
    result = find_disease_from_variant(chromosome, position, ref_allele, alt_allele, email, source_build="GRCh38")
